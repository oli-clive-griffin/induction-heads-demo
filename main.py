...
class AttentionLayer(nn.Module):
    def __init__(self, d_model, d_head, index) -> None:
        ...
        self.index = index
        self.d_head = d_head
        self.Q1 = nn.Linear(d_model, d_head)
        self.K1 = nn.Linear(d_model, d_head)
        self.V1 = nn.Linear(d_model, d_head)
        self.O1 = nn.Linear(d_head, d_model)
    ...
    def forward(self, x: Tensor, with_cache):
        ...
        Q = self.Q1.forward(x) # (batch_size, seq_len, d_head)
        K = self.K1.forward(x) # (batch_size, seq_len, d_head)
        V = self.V1.forward(x) # (batch_size, seq_len, d_head)
...
QK = model.attn1.Q1.weight.T @ model.attn1.K1.weight
print(QK)
imshow(QK.detach().cpu().numpy())
# imshow(model.attn1.V1.weight.T.detach().cpu().numpy())
# %%

OV_l1 = model.attn1.O1.weight @ model.attn1.V1.weight
OV_l2 = model.attn2.O1.weight @ model.attn2.V1.weight
...
# %%
QK = model.attn2.Q1.weight.T @ model.attn2.K1.weight
imshow(QK.detach().cpu().numpy())
# %%
...
model.attn1.O1.weight = saved1
model.attn2.O1.weight = saved2
#%%

model.attn1.O1.weight = nn.Parameter(torch.zeros_like(model.attn1.O1.weight))
model.attn2.O1.weight = nn.Parameter(torch.zeros_like(model.attn2.O1.weight))
